...
_SYSTEM_PROMPT = (
    ...
    "\n"
    "[문체와 태도]\n"
    "- 실제 법원 판결문의 문어체 존댓말을 씁니다. 예: '본 재판부는 ~', '~한 것으로 판단된다', "
    "'~라 아니할 수 없다', '~에 처한다', '~을 명한다'.\n"
    "- 근엄한 척하지만 한 방에 뼈를 때리는 유머가 핵심입니다. 판결문 형식 안에서 피식 웃게 만드십시오.\n"
    "- 놀림의 대상은 오직 '소비 습관'입니다. 외모·체형·건강·성격·소득 수준은 언급 금지.\n"
    "- 칭찬(무혐의) 톤 지시가 오면 비꼬지 말고 담백하게 인정하고 응원합니다.\n"
    "\n"
    "[이유(reasoning)]\n"
    "- 정확히 2문장, 각 문장 45자 내외.\n"
    "- 제공된 수치(총지출, 카테고리별 금액·비율·건수, 결제 건수, 소액 건수 등) 중 최소 1개를 그대로 인용.\n"
    "- 죄명이 왜 성립하는지를 그 수치로 뒷받침합니다.\n"
    "- '정상참작' 항목이 제공되면 반드시 이유 안에서 유쾌하게 언급합니다.\n"
    "- 제공되지 않은 수치·상호·사실을 지어내지 않습니다.\n"
    "\n"
    "[형량(sentence)]\n"
    "- 1문장. 이번 소비 패턴에 딱 맞춘 창의적이고 유쾌한 형벌 문구.\n"
    "- 명령형 존댓말로 끝냅니다. 예: '~에 처한다.', '~를 명한다.', '~하십시오.'\n"
    "- 실행 가능한 소소한 미션 형태가 좋습니다. (예: 배달앱 아이콘 폴더 맨 뒤로 추방 3일)\n"
    "\n"
    "[출력 형식]\n"
...
_FEWSHOT: list[dict] = [
    ...
    },
    {
        ...
        "content": json.dumps({
            "reasoning": (
                "피고인은 이번 달 배달·외식에 184,000원을 지출하여 전체 소비의 42.6%를 한곳에 몰아넣었다. "
    ...
    },
    {
        ...
        "content": json.dumps({
            "reasoning": (
                "식재료·생필품에 152,000원, 전체의 39.2%를 배정하고 배달은 4건에 그친 점이 확인된다. "
...
def detect_split_bill(expenses: list[dict]) -> dict | None:
    """memo에서 N빵·더치페이 정황을 찾아 정상참작 정보를 만든다.

    Returns
    -------
    dict | None
        {"note": 화면·프롬프트용 문구, "matchedCount": int,
         "matchedTotal": int, "estimatedBurden": int}
    """
    if not expenses:
        return None
    ...
    for e in expenses:
        memo = str(e.get("memo", "")).lower()
        if not memo:
            continue
        if any(k in memo for k in _SPLIT_KEYWORDS):
            matched.append(e)
            m = _HEADCOUNT_RE.search(memo)
            if m:
                n = int(m.group(1))
                if 2 <= n <= 30:
...
def _mock_verdict(stats: dict, consumer_type: dict, judgment: dict,
    ...
    if consumer_type["code"] in _PRAISE_TYPES:
        reasoning = (
            f"{head} 등에 비추어 계획적이고 절제된 소비 습관이 인정된다. "
            f"피고인을 '{label}'으로 판단함이 상당하다.{mitig}"
        )
        sentence = "현재의 소비 습관을 그대로 유지할 것을 명하며, 본 재판부가 조용히 응원한다."
    else:
        reasoning = (
            f"{head} 등의 정황에 비추어 특정 소비에 편중된 지출 습관이 뚜렷하다. "
            f"이를 '{label}'으로 판단함이 상당하다.{mitig}"
        )
        sentence = "다음 달 동일 카테고리 지출을 주 2회 이하로 제한하고, 무지출 데이라 1회를 명한다."
...
def _call_bedrock(stats: dict, consumer_type: dict, judgment: dict,
    ...
    messages = _FEWSHOT + [
        {"role": "user",
         "content": _build_user_message(stats, consumer_type, judgment, split)}
    ]
    body = json.dumps({
    ...
    return _parse_verdict(result["content"][0]["text"])
...
def generate_verdict(stats: dict, consumer_type: dict, judgment: dict,
    ...
    expenses : list[dict]   fetch_month_expenses() 반환값 (memo 정상참작용, 없으면 생략)
    ...
    if os.environ.get("MOCK_AI") == "1":
        out = _mock_verdict(stats, consumer_type, judgment, split)
        out["mitigation"] = split
        return out

    for attempt in range(2):
        try:
            out = _call_bedrock(stats, consumer_type, judgment, split)
            out["mitigation"] = split
            return out
...
def generate_reasoning(stats: dict, consumer_type: dict, judgment: dict) -> str:
    ...
    return generate_verdict(stats, consumer_type, judgment).get("reasoning", "")
