...
# (store_name, date, amount, category, transaction_type[, plea])
# plea는 선택 — 피고인 변론(N빵·더치페이 메모)이 있으면 판결문에서 정상참작으로 반영된다.
SEED_DATA: list[tuple] = [
    ...
    ("쿠팡이츠", "2026-08-09", 45000, "DELIVERY_DINING", "EXPENSE", "친구 4명이서 시켜서 더치페이함"),
    ...
    # INCOME (분석 제외 — 달력에 +초록으로 표시되는 데모용)
    ("부모님 용돈", "2026-08-01", 300000, "OTHER", "INCOME"),
    ("통장 이자", "2026-08-25", 1200, "OTHER", "INCOME"),
...
def run_seed():
    ...
    for row in SEED_DATA:
        ...
        conn.execute(
            ...
            (store_name, date, amount, category, tx_type, DEFAULT_DEFENDANT, plea),
        )
        inserted += 1
