...
MOCK_AI=1 경로, Bedrock 경로(_call_bedrock 몽키패치), 폴백, 정상참작 감지를 모두 검증한다.
"""
import json
import os
import sys
...
class TestDetectSplitBill(unittest.TestCase):
    def test_none_when_no_memo(self):
        self.assertIsNone(vs.detect_split_bill([]))
        self.assertIsNone(vs.detect_split_bill([{"storeName": "배민", "amount": 30000, "memo": ""}]))

    def test_detects_dutch_pay(self):
        exp = [{"storeName": "쿠팡이츠", "amount": 45000, "memo": "친구 4명 더치페이"}]
    ...
    def test_default_headcount_two(self):
        exp = [{"storeName": "포차", "amount": 50000, "memo": "n빵함"}]
        out = vs.detect_split_bill(exp)
    ...
    def test_multiple_matches_sum(self):
        exp = [
            {"storeName": "A", "amount": 30000, "memo": "3명 갹출"},
            {"storeName": "B", "amount": 20000, "memo": "회비"},
            {"storeName": "C", "amount": 99999, "memo": "그냥 혼밥"},
        ]
        out = vs.detect_split_bill(exp)
...
class TestMockPath(unittest.TestCase):
    ...
    def test_returns_reasoning_and_sentence(self):
        out = vs.generate_verdict(_STATS, _CT, _JUDGMENT, [])
    ...
    def test_mock_reflects_mitigation(self):
        exp = [{"storeName": "쿠팡이츠", "amount": 45000, "memo": "친구 4명 더치페이"}]
        out = vs.generate_verdict(_STATS, _CT, _JUDGMENT, exp)
    ...
    def test_praise_tone(self):
        ...
        out = vs.generate_verdict(_STATS, ct, _JUDGMENT, [])
        self.assertIn("응원", out["sentence"])

    def test_backcompat_generate_reasoning(self):
        self.assertTrue(vs.generate_reasoning(_STATS, _CT, _JUDGMENT))
...
class TestBedrockPath(unittest.TestCase):
    ...
    def test_mitigation_passed_through_on_fallback(self):
        exp = [{"storeName": "포차", "amount": 60000, "memo": "5명 n빵"}]
